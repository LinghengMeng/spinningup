...
class BootstrappedActorCriticEnsemble():
    ...
    def _create_actor_critic(self, ac_i):
        ...
        return dict(replay_buf=replay_buf,
                    obs_ph=obs_ph, act_ph = act_ph, rew_ph = rew_ph, new_obs_ph = new_obs_ph, done_ph = done_ph,
        ...
        for ac_i in range(self.ensemble_size):
            if np.random.uniform(0, 1, 1) <= self.replay_buf_bootstrap_p:
                self.ensemble[ac_i]['replay_buf'].store(obs, act, rew, next_obs, done,
                                                        step_index, epoch_index, time_stamp, **kwargs)
        # print('add_to_replay_buffer time: {}'.format(time.time()-start))
    ...
    def _uncertainty_based_mini_batch(self, ac_i, batch_size, raw_batch_size):
        raw_batch = self.ensemble[ac_i]['replay_buf'].sample_batch(raw_batch_size)
