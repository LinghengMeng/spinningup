...
import gym
import time
from spinup.algos.dbedpg_shared_replay_buff import core
from spinup.algos.dbedpg_shared_replay_buff.core import get_vars, BootstrappedActorCriticEnsemble
...
"""
def dbedpg_shared_replay_buff(env_name, render_env=False, ac_kwargs=dict(), seed=0,
    ...
    def get_action(o, noise_scale, ac_i=0):
        ...
        if policy_type == 'act_with_highest_predicted_q_without_act_noise':
        ...
        elif policy_type == 'act_with_highest_predicted_q_with_act_noise':
            # Select action with the highest predicted Q-value
            descending_sort_q = np.argsort(-ac_ensemble_preds_q) # Descending sort
            a = ac_ensemble_preds_a[descending_sort_q[0], :]
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            logger.store(EpRet=ep_ret, EpLen=ep_len, EpVarQ=ep_var_q)
            o, r, d, ep_ret, ep_len, ep_var_q = env.reset(), 0, False, 0, 0, 0
        ...
        if t > 0 and t % steps_per_epoch == 0:
            ...
            # TODO: select actor based on test episode return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy_type', type=str,
                        choices=['act_with_highest_predicted_q_without_act_noise',
                                 'act_with_highest_predicted_q_with_act_noise',
                                 'random_ac_for_each_epoch', 'sample_based_on_action_covariance'],
    ...
    dbedpg_shared_replay_buff(env_name=args.env, render_env=args.render_env,
