...
def mlp_actor_critic(x, a, hidden_sizes=(400,300), activation=tf.nn.relu, 
    ...
    if nn_type == 'mlp':
        ...
        with tf.variable_scope('q1'):
            q1 = tf.squeeze(mlp_dropout(tf.concat([x,a], axis=-1), list(hidden_sizes)+[1], activation, None, dropout_rate), axis=1)
        with tf.variable_scope('q2'):
            q2 = tf.squeeze(mlp_dropout(tf.concat([x,a], axis=-1), list(hidden_sizes)+[1], activation, None, dropout_rate), axis=1)
        with tf.variable_scope('q1', reuse=True):
            q1_pi = tf.squeeze(mlp_dropout(tf.concat([x,pi], axis=-1), list(hidden_sizes)+[1], activation, None, dropout_rate), axis=1)
    elif nn_type == 'mlp_variational':
        with tf.variable_scope('pi'):
            pi, _ = mlp_variational(x, list(hidden_sizes) + [act_dim], activation, output_activation, dropout_rate)
            pi = act_limit * pi
        with tf.variable_scope('q1'):
            q1, _ = mlp_variational(tf.concat([x, a], axis=-1), list(hidden_sizes) + [1], activation, None,
                                    dropout_rate)
        with tf.variable_scope('q2'):
            q2, _ = mlp_variational(tf.concat([x, a], axis=-1), list(hidden_sizes) + [1], activation, None,
                                    dropout_rate)
        with tf.variable_scope('q1', reuse=True):
            q1_pi, _ = mlp_variational(tf.concat([x, pi], axis=-1), list(hidden_sizes) + [1], activation, None,
                                       dropout_rate)
    else:
