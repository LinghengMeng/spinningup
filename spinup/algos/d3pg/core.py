...
class MLP(tf.keras.Model):
    ...
    def call(self, inputs, duplicate_input = False):
        if duplicate_input:
            x = tf.concat([inputs, inputs**2], axis=1)
        else:
            x = inputs
...
class DuelingMLP(tf.keras.Model):
    """
    Multi-Layer Perceptron Network:
        Model class used to create mlp.
    """
    def __init__(self, v_h_size = 300, v_out_size = 1, a_h_size = 300, a_out_size = 1,
                 kernel_initializer='glorot_uniform', bias_initializer='zeros',
                 hidden_activation=tf.keras.activations.relu, output_activation=tf.keras.activations.linear):
        super(DuelingMLP, self).__init__()
        # state-value function
        self.v_h1 = tf.keras.layers.Dense(64, activation=hidden_activation,
                                          kernel_initializer=kernel_initializer,
                                          bias_initializer=bias_initializer)
        self.v_h2 = tf.keras.layers.Dense(64, activation=hidden_activation,
                                          kernel_initializer=kernel_initializer,
                                          bias_initializer=bias_initializer)
        self.v_out = tf.keras.layers.Dense(v_out_size, activation=output_activation,
                                           kernel_initializer=kernel_initializer,
                                           bias_initializer=bias_initializer)
        # advantage-value function
        self.a_h1 = tf.keras.layers.Dense(100, activation=hidden_activation,
                                          kernel_initializer=kernel_initializer,
                                          bias_initializer=bias_initializer)
        self.a_h2 = tf.keras.layers.Dense(100, activation=hidden_activation,
                                          kernel_initializer=kernel_initializer,
                                          bias_initializer=bias_initializer)
        self.a_out = tf.keras.layers.Dense(a_out_size, activation=output_activation,
                                           kernel_initializer=kernel_initializer,
                                           bias_initializer=bias_initializer)

    def call(self, inputs, state, action, optimal_action):
        # state-value function
        v_h1_out = self.v_h1(state)
        v_h2_out = self.v_h2(v_h1_out)
        v_out = self.v_out(v_h2_out)
        # advantage-value function
        # merged_v_a = tf.concat([v_h1_out, action], axis=1)
        merged_v_a = tf.concat([state, action], axis=1)
        a_h1_out = self.a_h1(merged_v_a)
        a_h2_out = self.a_h2(a_h1_out)
        a_out = self.a_out(a_h2_out)
        # aggregated q-value function
        # merged_v_optimal_a = tf.concat([v_h1_out, optimal_action], axis=1)
        merged_v_optimal_a = tf.concat([state, optimal_action], axis=1)
        optimal_a_h1_out = self.a_h1(merged_v_optimal_a)
        optimal_a_h2_out = self.a_h2(optimal_a_h1_out)
        optimal_a_out = self.a_out(optimal_a_h2_out)
        # q_out = v_out + (a_out-optimal_a_out)
        q_out = v_out + a_out
        return v_out, a_out, q_out
...
class MLPAleatoricUnc(tf.keras.Model):
    ...
    def __init__(self, layer_sizes=[32],
        ...
        self.out_layer = tf.keras.layers.Dense(layer_sizes[-1], activation=output_activation,
                                               ...
                                               bias_initializer=bias_initializer)
        # Log(sigma^2)
        self.out_layer_aleatoric_unc = tf.keras.layers.Dense(layer_sizes[-1], activation=output_activation,
                                                             kernel_initializer=kernel_initializer,
                                                             bias_initializer=bias_initializer)
...
class BeroulliDropoutMLP(tf.keras.Model):
    ...
    def call(self, inputs, training=None, duplicate_input = False, mask=None):
        if duplicate_input:
            x = tf.concat([inputs, inputs**2], axis=1)
        else:
            x = inputs
