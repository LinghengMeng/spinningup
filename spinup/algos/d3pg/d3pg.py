...
import gym
import time
from spinup.algos.d3pg import core
from spinup.algos.d3pg.core import DuelingMLP, MLP, MLPAleatoricUnc, BeroulliDropoutMLP, get_vars, count_vars
...
"""
def d3pg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main'):
        ...
        v_out, a_out, q_out = dueling_critic(None, state=x_ph, action=a_ph, optimal_action=pi)
        v_out, a_out, q_out = tf.squeeze(v_out, axis=1), tf.squeeze(a_out, axis=1), tf.squeeze(q_out, axis=1)
        _, a_pi_out, q_pi_out = dueling_critic(None, state=x_ph, action=pi, optimal_action=pi)
        a_pi_out, q_pi_out = tf.squeeze(a_pi_out, axis=1), tf.squeeze(q_pi_out, axis=1)
    ...
    with tf.variable_scope('target'):
        ...
        _, a_pi_out_targ, q_pi_out_targ = dueling_critic(None, state=x2_ph, action=pi_targ, optimal_action=pi)
    ...
    backup = tf.stop_gradient(r_ph*reward_scale + gamma*(1-d_ph)*q_pi_out_targ)
    ...
    train_q_op = q_optimizer.minimize(q_loss, var_list=dueling_critic.variables)
        ...
        if d or (ep_len == max_ep_len):
            ...
            if not without_delay_train:
                for j in range(ep_len):
                    ...
                    outs = sess.run(q_step_ops, feed_dict)
                    logger.store(LossQ=outs[0], VVals=outs[1], AVals=outs[2], QVals=outs[3])
...
if __name__ == '__main__':
    ...
    d3pg(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
