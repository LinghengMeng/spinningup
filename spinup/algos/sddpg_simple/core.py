...
Actor-Critics
"""
def mlp_actor_critic(x, a_mu_ph, a_sigma_ph, hidden_sizes=(400,300), activation=tf.nn.relu,
    ...
    with tf.variable_scope('pi'):
        ...
        pi_mu, pi_sigma, pi = a_mlp(x, list(hidden_sizes) + [act_dim], activation, output_activation)
        pi = act_limit * pi
    with tf.variable_scope('q'):
        q = tf.squeeze(mlp(tf.concat([x, a_mu_ph, a_sigma_ph], axis=-1), list(hidden_sizes)+[1], activation, None), axis=1)
        # q = q_mlp(tf.concat([x, a], axis=-1), list(hidden_sizes)+[1], activation, None)
    with tf.variable_scope('q', reuse=True):
        q_pi = tf.squeeze(mlp(tf.concat([x, pi_mu, pi_sigma], axis=-1), list(hidden_sizes)+[1], activation, None), axis=1)
        # q_pi = q_mlp(tf.concat([x,pi], axis=-1), list(hidden_sizes)+[1], activation, None)
    return pi, pi_mu, pi_sigma, q, q_pi
