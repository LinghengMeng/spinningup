...
import gym
import time
from spinup.algos.sddpg_simple import core
from spinup.algos.sddpg_simple.core import get_vars
...
"""
def sddpg_simple(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main'):
        pi, pi_mu, pi_sigma, q, q_pi = actor_critic(x_ph, a_mu_ph, a_sigma_ph, **ac_kwargs)
        # pi, q, q_mu, q_sigma, q_pi, q_pi_mu, q_pi_sigma = actor_critic(x_ph, a_ph, **ac_kwargs)
    # Target networks
    with tf.variable_scope('target'):
        # Note that the action placeholder going to actor_critic here is 
        # irrelevant, because we only need q_targ(s, pi_targ(s)).
        pi_targ, pi_mu_targ, pi_sigma_targ, _, q_pi_targ = actor_critic(x2_ph, a_mu_ph, a_sigma_ph, **ac_kwargs)
    ...
    for t in range(total_steps):
        ...
        if t > start_steps:
            ...
            a_mu = env.action_space.sample()
            a_sigma = np.random.uniform(0,1,size=act_dim)
            # import pdb; pdb.set_trace()
            conc_factor = 1
            a = np.random.normal(a_mu, conc_factor*a_sigma)
...
if __name__ == '__main__':
    ...
    sddpg_simple(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
