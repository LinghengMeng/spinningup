...
import gym
import time
from spinup.algos.sddpg2 import core
from spinup.algos.sddpg2.core import get_vars
...
class ReplayBuffer:
    ...
    def store(self, obs, act, act_mu, act_alpha, act_beta, rew, next_obs, done,
        ...
        self.log_experiences(obs, act, act_mu, act_alpha, act_beta, rew, next_obs, done,
                             step_index, steps_per_epoch, start_time)
    ...
    def log_experiences(self, obs, act, act_mu, act_alpha, act_beta, rew, next_obs, done,
                        step_index, steps_per_epoch, start_time):
...
"""
def sddpg2(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main'):
        pi, pi_mu, pi_sigma, pi_rho, pi_cov, pi_corr_iter_number, q, q_pi = actor_critic(x_ph, a_ph, **ac_kwargs)
        # pi, q, q_mu, q_sigma, q_pi, q_pi_mu, q_pi_sigma = actor_critic(x_ph, a_ph, **ac_kwargs)
    # Target networks
    with tf.variable_scope('target'):
        # Note that the action placeholder going to actor_critic here is 
        # irrelevant, because we only need q_targ(s, pi_targ(s)).
        pi_targ, pi_mu_targ, pi_sigma_targ, pi_rho_targ, pi_cov_targ, _, _, q_pi_targ = actor_critic(x2_ph, a_ph, **ac_kwargs)
    ...
    logger.setup_tf_saver(sess, inputs={'x': x_ph, 'a': a_ph},
                          outputs={'pi_mu': pi_mu, 'pi_sigma': pi_sigma, 'pi_beta': pi_rho, 'q': q})
    ...
    def get_action(o):
        ...
        a, a_mu, a_alpha, a_beta, a_cov, a_corr_iter_number = sess.run([pi, pi_mu, pi_sigma, pi_rho, pi_cov, pi_corr_iter_number],
                                                   feed_dict={x_ph: o.reshape(1, -1)})
    ...
    for t in range(total_steps):
        ...
        replay_buffer.store(o, a, a_mu, a_alpha, a_beta, reward_scale*r, o2, d,
        ...
        if d or (ep_len == max_ep_len):
            ...
            for j in range(ep_len):
                if j % 100 == 0:
                    train_end = time.time()
                    print('training step={}, cost_time={}'.format(j, train_end-train_start))
                ...
                # Q-learning update
                outs = sess.run([q_loss, q, train_q_op,pi_corr_iter_number], feed_dict)
                ...
                if j % policy_delay == 0:
                    ...
                    outs = sess.run([pi_loss, train_pi_op, target_update,pi_corr_iter_number], feed_dict)
...
if __name__ == '__main__':
    ...
    sddpg2(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
