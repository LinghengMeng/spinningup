...
def a_mlp(x, hidden_sizes=(32,), activation=tf.nn.relu, output_activation=tf.tanh):
    ...
    # alpha and beta for information matrix
    ...
    a_cov, a_corr = construct_covariance_matrix(a_sigma, a_rho, act_dim)

    a_dist = tfp.edward2.MultivariateNormalFullCovariance(a_mu, a_cov)
    
    # a = tf.squeeze(a_dist.sample([1]), axis=0)
    a = a_dist
    a = tf.tanh(a) # shift back to [-1,1]
    return a, a_mu, a_sigma, a_rho, a_cov

def multivariate_normal(mu, cov):
    pass
