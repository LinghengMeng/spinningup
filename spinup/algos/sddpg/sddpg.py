...
import gym
import time
from spinup.algos.sddpg import core
from spinup.algos.sddpg.core import get_vars
...
"""
def sddpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    x2_ph, r_ph, d_ph = core.placeholders(obs_dim, act_dim, act_dim, act_dim, (act_dim*(act_dim-1))/2, obs_dim, None, None)

    # Main outputs from computation graph
    with tf.variable_scope('main'):
        pi, pi_mu, pi_alpha, pi_beta, pi_cov, q, q_pi = actor_critic(x_ph, a_mu_ph, a_alpha_ph, a_beta_ph, **ac_kwargs)
        # pi, q, q_mu, q_sigma, q_pi, q_pi_mu, q_pi_sigma = actor_critic(x_ph, a_ph, **ac_kwargs)
    # Target networks
    with tf.variable_scope('target'):
        # Note that the action placeholder going to actor_critic here is 
        # irrelevant, because we only need q_targ(s, pi_targ(s)).
        pi_targ, pi_mu_targ, pi_alpha_targ, pi_beta_targ, pi_cov_targ, _, q_pi_targ = actor_critic(x2_ph, a_mu_ph, a_alpha_ph, a_beta_ph, **ac_kwargs)
    ...
    def get_action(o):
        # import pdb; pdb.set_trace()
        a, a_mu, a_alpha, a_beta, a_cov = sess.run([pi, pi_mu, pi_alpha, pi_beta, pi_cov],feed_dict={x_ph: o.reshape(1,-1)})
...
if __name__ == '__main__':
    ...
    sddpg(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
